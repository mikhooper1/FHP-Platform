...
export default function Home() {
  ...
  const activeLesson: Lesson | null = lessons.find(l => l.week === `Week ${currentWeek}` && l.status === 'published')
  ...
  useEffect(() => {
    ...
    if (savedSummary) setMirrorSummary(savedSummary)
  }, [])
  ...
  try { mirrorData = mirrorSummary ? JSON.parse(mirrorSummary) : null } catch {}
  ...
  const btnPrimary: React.CSSProperties = {
    ...
    padding: '11px 22px', fontFamily: 'Barlow Condensed',
    fontSize: 10, fontWeight: 500, letterSpacing: '0.2em',
    textTransform: 'uppercase' as const, cursor: 'pointer',
    borderRadius: 7, background: 'var(--orange)',
    border: '1.5px solid var(--orange)', color: 'white',
  ...
  const divider: React.CSSProperties = {
    height: 1, background: 'var(--cream3)', margin: '28px 0'
  }
        ...
        <div style={{ width: '100%', maxWidth: 400 }}>
          ...
          {obStep === 1 && (
            <div>
              ...
              <input style={{ width: '100%', background: 'var(--white)', border: '1px solid var(--cream4)', borderRadius: 7, padding: '13px 15px', fontFamily: 'Barlow', fontSize: 15, color: 'var(--ink)', outline: 'none', marginBottom: 10, display: 'block' }}
                placeholder="Your first name" value={athleteName} onChange={e => setAthleteName(e.target.value)} />
              <input style={{ width: '100%', background: 'var(--white)', border: '1px solid var(--cream4)', borderRadius: 7, padding: '13px 15px', fontFamily: 'Barlow', fontSize: 15, color: 'var(--ink)', outline: 'none', marginBottom: 16, display: 'block' }}
                placeholder="Your email address" type="email" value={athleteEmail} onChange={e => setAthleteEmail(e.target.value)} />
              <button onClick={handleBegin} style={{ ...btnPrimary, width: '100%', justifyContent: 'center', marginBottom: 10 }}>Begin →</button>
              <p style={{ fontSize: 11, color: 'var(--ink4)', textAlign: 'center', lineHeight: 1.6 }}>By continuing you agree to receive FHP program updates. Unsubscribe anytime.</p>
            </div>
          )}
          ...
          {obStep === 2 && (
            <div>
              <h1 style={{ fontFamily: 'Barlow Condensed', fontSize: 24, fontWeight: 300, color: 'var(--ink)', marginBottom: 8, textAlign: 'center' }}>Where are you right now?</h1>
              <p style={{ fontSize: 13.5, fontWeight: 300, color: 'var(--ink3)', lineHeight: 1.7, textAlign: 'center', marginBottom: 24 }}>This shapes your experience.</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 20 }}>
                {[['Academy', 'Development program'], ['Semi-pro', 'Transitioning to pro'], ['Professional', 'Contracted athlete'], ['School sport', 'Representative level']].map(([name, sub]) => (
                  <div key={name} onClick={() => handleLevelSelect(name)}
                    style={{ border: `1.5px solid ${athleteLevel === name ? 'var(--orange)' : 'var(--cream4)'}`, background: athleteLevel === name ? 'var(--orange-t)' : 'var(--white)', borderRadius: 8, padding: '14px', cursor: 'pointer', transition: 'all 0.12s' }}>
          ...
          {obStep === 3 && (
            <div>
              <h1 style={{ fontFamily: 'Barlow Condensed', fontSize: 24, fontWeight: 300, color: 'var(--ink)', marginBottom: 8, textAlign: 'center' }}>Here&apos;s how FHP works.</h1>
              <p style={{ fontSize: 13.5, fontWeight: 300, color: 'var(--ink3)', lineHeight: 1.7, textAlign: 'center', marginBottom: 24 }}>Three simple things. Done consistently.</p>

              <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 28 }}>
                {[
                  ['01', 'Watch the lesson', 'Each week has one short video and a framework to work through. It takes 20–25 minutes.'],
                  ['02', 'Reflect daily', 'Answer one question at the end of each day. Honest, short entries. Takes 5 minutes.'],
                  ['03', 'Read your mirror', 'The AI reads your entries and builds a picture of how you actually operate under pressure. It updates weekly.'],
                ].map(([num, title, desc]) => (
                  <div key={num} style={{ display: 'flex', gap: 16, padding: '16px', background: 'var(--white)', border: '1px solid var(--cream3)', borderRadius: 10 }}>
                    ...
                    <div>
                      <div style={{ fontFamily: 'Barlow Condensed', fontSize: 14, fontWeight: 500, color: 'var(--ink)', marginBottom: 4, letterSpacing: '0.02em' }}>{title}</div>
  ...
  const navItems = [
    ...
    { id: 'lesson', label: 'This week', icon: '▷' },
    { id: 'journal', label: 'Reflect', icon: '✎' },
    { id: 'mirror', label: 'My mirror', icon: '◎' },
  ]
    ...
    <div style={{ display: 'flex', minHeight: '100vh', maxWidth: 1000, margin: '0 auto' }}>
      ...
      <nav style={{ width: 184, flexShrink: 0, background: 'var(--white)', borderRight: '1px solid var(--cream3)', display: 'flex', flexDirection: 'column', padding: '28px 0 24px', position: 'sticky', top: 0, height: '100vh' }}
        ...
        <div style={{ padding: '0 18px 22px', borderBottom: '1px solid var(--cream3)', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ fontFamily: 'Barlow Condensed', fontSize: 9, fontWeight: 400, letterSpacing: '0.2em', color: 'var(--ink3)', textTransform: 'uppercase', lineHeight: 1.5 }}>Athlete<br />Platform</div>
        </div>

        {navItems.map(({ id, label }) => (
          <div key={id} onClick={() => navTo(id as Page)}
            style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 18px', cursor: 'pointer', fontFamily: 'Barlow Condensed', fontSize: 13.5, letterSpacing: '0.04em', color: page === id ? 'var(--orange)' : 'var(--ink3)', borderLeft: `2px solid ${page === id ? 'var(--orange)' : 'transparent'}`, marginBottom: 2, transition: 'color 0.12s', background: page === id ? 'var(--orange-t)' : 'transparent' }}>
            {label}
          </div>
        ))}
        ...
        <div style={{ margin: '20px 18px 0', padding: '14px', background: 'var(--cream)', borderRadius: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 6 }}>
            {[1, 2, 3, 4].map(w => (
              <div key={w} style={{ flex: 1, height: 3, borderRadius: 2, background: w <= currentWeek ? 'var(--orange)' : 'var(--cream3)', transition: 'background 0.3s' }} />
            ))}
          </div>
        ...
        {page === 'home' && (
          <div>
            ...
            <div style={section}>
              ...
              <div style={{ fontFamily: 'Barlow Condensed', fontSize: 22, fontWeight: 600, letterSpacing: '0.01em', color: 'var(--ink)', lineHeight: 1.2, marginBottom: 16 }}>
            ...
            <div style={section}>
              ...
              <div style={{ fontFamily: 'Barlow Condensed', fontSize: 16, fontWeight: 500, letterSpacing: '0.02em', color: 'var(--ink)', lineHeight: 1.45 }}>
                {activeLesson?.action ?? '—'}
              </div>
            </div>
            ...
            <div style={section}>
              <span style={eyebrow()}>
                From your mirror
                <span style={{ fontSize: 9, color: 'var(--ink4)', fontWeight: 400, marginLeft: 8, letterSpacing: '0.08em', textTransform: 'none' }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: 'Barlow Condensed', fontSize: 16, fontWeight: 400, color: 'var(--ink)', marginBottom: 14, letterSpacing: '0.02em' }}>
          ...
          <div style={{ maxWidth: 580 }}>
            {activeLesson ? (
              <>
                ...
                {/* Video placeholder (Vimeo to be re-enabled once privacy settings allow) */}
                ...
                <div style={{ background: 'var(--ink)', borderRadius: 14, padding: '28px 30px' }}>
                  ...
                  <div style={{ fontFamily: 'Barlow Condensed', fontSize: 17, fontWeight: 300, letterSpacing: '0.02em', color: 'white', lineHeight: 1.3, marginBottom: 16 }}>
        ...
        {page === 'journal' && (
          <div style={{ maxWidth: 540 }}>
            <div style={{ marginBottom: 36 }}>
              ...
              <h1 style={{ fontFamily: 'Barlow Condensed', fontSize: 28, fontWeight: 300, letterSpacing: '0.02em', color: 'var(--ink)' }}>Reflect.</h1>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 20 }}>
              {PROMPTS.map(q => (
                <button key={q} onClick={() => setJournalPrompt(q)}
                  style={{ fontFamily: 'Barlow Condensed', fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase' as const, padding: '6px 12px', border: `1px solid ${journalPrompt === q ? 'var(--orange)' : 'var(--cream4)'}`, borderRadius: 40, color: journalPrompt === q ? 'var(--orange)' : 'var(--ink3)', background: journalPrompt === q ? 'var(--orange-t)' : 'transparent', cursor: 'pointer', transition: 'all 0.12s' }}>
                  {q}
                </button>
              ))}
            </div>
            ...
            {!journalSaved ? (
              <>
                <div style={{ fontFamily: 'Barlow Condensed', fontSize: 19, fontWeight: 300, letterSpacing: '0.02em', color: 'var(--ink)', lineHeight: 1.25, marginBottom: 16 }}>{journalPrompt}</div>
        ...
        {page === 'mirror' && (
          <div style={{ maxWidth: 580 }}>
            <div style={{ marginBottom: 36 }}>
              ...
              <h1 style={{ fontFamily: 'Barlow Condensed', fontSize: 28, fontWeight: 300, letterSpacing: '0.02em', color: 'var(--ink)' }}>Your mirror.</h1>
              <p style={{ fontSize: 13, fontWeight: 300, color: 'var(--ink3)', marginTop: 6, lineHeight: 1.6 }}>
            ...
            {journalEntries.length < 3 ? (
              ...
              <div>
                <div style={{ ...card, textAlign: 'center', marginBottom: 16 }}>
                  ...
                  <div style={{ fontSize: 13.5, fontWeight: 300, color: 'var(--ink3)', lineHeight: 1.75, maxWidth: 320, margin: '0 auto 20px' }}>
                    Write {Math.max(0, 3 - journalEntries.length)} more reflection{3 - journalEntries.length !== 1 ? 's' : ''} to unlock your first AI analysis. Be honest — the AI picks up on specifics.
                  </div>
              ...
              <div>
                ...
                <div style={{ background: 'var(--ink)', borderRadius: 14, padding: '28px 30px', marginTop: 28 }}>
                  ...
                  <div style={{ fontFamily: 'Barlow Condensed', fontSize: 18, fontWeight: 300, letterSpacing: '0.02em', color: 'white', lineHeight: 1.3, marginBottom: 8 }}>
                ...
                <div style={{ marginTop: 24, textAlign: 'center' }}>
                  <button onClick={() => generateAiInsight(journalEntries)} style={btnOutline}>
                    Refresh mirror
                  </button>
                </div>
              </div>
            ) : (
              /* Has entries but no mirror yet — trigger generation */
      ...
      {/* ─── MOBILE BOTTOM NAV ─── */}
      <div style={{ display: 'none', position: 'fixed', bottom: 0, left: 0, right: 0, background: 'var(--white)', borderTop: '1px solid var(--cream3)', padding: '8px 0 env(safe-area-inset-bottom, 8px)', zIndex: 50 }} className="mobile-nav">
